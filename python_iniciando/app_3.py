...
if st.button("Calcular"):
    ...
    sigma = 56 if material_PE == "Cobre" else 36
    ...
    if amp:
        ...
        #Cantidad de voltaje que se puede perder
        DELTA_V_C = (P_CT * V_OC) / 100
        if mm2:
            awg = calibre_awg(mm2)
            S_C = (2 * L_C * Pot) / (sigma * DELTA_V_C * V_OC)
            if awg:
            ...
            st.write(f"Calibre recomendado de conductores portadores de corriente CD Por Caida de Tensión: {awg} AWG ({mm2} mm²) ")
            #st.write(f"EL tamaño del conductor en AWG es: {awg}")
